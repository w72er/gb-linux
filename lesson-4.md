...
a@gb-udt:~$ ls | wc -l
a@gb-udt:~$ cat /etc/passwd | grep nobody
a@gb-udt:~$ ps -ef | less
```

## Мониторинг процессов и состояния компьютера.

Поскольку линукс - серверная ОС, где вопрос мониторинга стоит
остро, поэтому там множество утилит.

Есть сложные решения, агенты собирают информацию, которые отсылают
на сервер

`ps` - список запущенных процессов.

`top` (table of process) - выведет список процессов, но отличается
от `top` периодичностью обновления данных. Данные выводятся в двух
частях - верхняя - общая информация, в нижней части информация по
отдельным процессам.
Если наша система стала подозрительна медленной, то запускаем `top`
и смотрим на потребляемую %mem %cpu.

`df` - показывает смонтированные файловые системы. Для
человекочитаемого вида используем `-h`.

udev - аппаратные ресурсы отображаются на файлы.
tmpfs - временная файловая система
/dev/... - реальные разделы жесткого диска
/dev/loop - образы которые подмонтированы в пакеты snap

2. перед командой `cut` использовать `ls -l`, чтобы отфильтровать
все повторяющие строки
   
3. Найти строку с параметром и проставить для него no, но.
Сравнить идентификаторы процессов.

Запустить оставьте -15 сигнал. Оба варианта будут приняты.
